...
import java.io.*;
    ...
    /*  Programmer: Solomon Davis, Lewis Roe
        Alias: Spiderman
        Due Date: Tuesday, December 2, 2019
        Course: Advanced Object Oriented Programming (CS 3331)
        Instructor: Daniel Meija
        Lab Assignment: Programming Assignment 5
        Lab Description: A user is able to log into the system
                         and perform specific actions based on if they're
                        a customer or a bank manager.
...
public class MainBank4 {
    ...
    /**
     * Main gets user choice and implements it based on the input from the keyboard.
     * After the user is done the program asks them if they want to continue
     * If the answer is yes the program asks th user to login.If the answer is no
    ...
    /**
     * Reads data from file into hash map. The name is used as the key for finding specific customers.
     * @param fileName
     * @return
     */
    ...
    /**
     * Reads data from transaction file into program and then carries out the desired action.
     * @param fileName
     * @param h
     * @param log
     * @param bs
    ...
    public static void updatedFile(String fileName, HashMap<String, Person> h) {//Lewis Roe
		...
		try {
			...
			newFile.println(header[firstNameIndex] + "," + header[lastNameIndex] + "," + header[DOB_index] + ","
                    ...
                    //gets from hash and adds to file
                    while (iterator.hasNext()) {

                        String name = (String) iterator.next();
                        Person currentUser = h.get(name);
                        CreditCard cc = (CreditCard) h.get(name).getCreditCard();

                        newFile.println(currentUser.getFirstName() + "," + currentUser.getLastName() + "," + currentUser.getDOB() + ","
                                + currentUser.getEmail() + "," + currentUser.getID() + "," + currentUser.getPassword() + "," + currentUser.getAddress()
                                + "," + currentUser.getPhoneNum() + "," + currentUser.getChecking().getAccountNum() + ","
                                + currentUser.getSavings().getAccountNum() + "," + currentUser.getCreditCard().getAccountNum() + ","
                                + currentUser.getChecking().getAccountBalance() + "," + currentUser.getSavings().getAccountBalance() + ","
                                + currentUser.getCreditCard().getAccountBalance()
                                + "," + cc.getCreditMax());
    ...
    /**
     * Adds a new customer to the hash map.
     * @param h
     */
    ...
    /**
     * Asks and performs the options of a regular customer.
     * Options include deposit,withdrawal, transfer, and payment.
     * Inquire is not a option since the customer information is displayed after
     * every transaction.
     * @param user
     * @param h
     * @param log
    ...
    /**
     * Gives the bank manager the option inquire about a specific customer's data 
     * using the name of the customer. If the bank manager wants a specific account
     * they have to the account number specific to that account.
     * @param h
     */
    ...
    /**
     * Displays all the information of all the customers in the bank.
     * @param h
     */
    ...
    /**
     * Gets and performs bank manager's choice. The bank manager has the option of
     * Displaying all customer info, getting information about a specific customer, 
     * writing a bank statement for a specific customer, adding a new customer, 
     * and reading from a transaction file.
     * @param h
     * @param fileName
     * @param log
     * @param bs
    ...
    /**
     * Writes bank statement for whatever customer the bank manager chooses.
     * @param h
     * @param bs
     * @throws IOException
     */
    public static void WriteBankStatement(HashMap<String, Person> h, BankStatement bs) throws IOException {//Solomon Davis
        ...
        if(customerName.equals(h.get(customerName).firstName +h.get(customerName).lastName)){
            ...
            //Calculates starting balance for bankstatement file
